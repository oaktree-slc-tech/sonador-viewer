...
class OHIFCornerstoneViewportOverlay extends PureComponent {
  ...
  render() {
    const { imageId, scale, windowWidth, windowCenter, inconsistencyWarnings, SRLabels } = this.props;
    ...
    const getSRLabelsInfo = (SRLabels) => {
      return (
        <React.Fragment>
          {SRLabels.length != 0 ? getSRLabelsContent(SRLabels) : <React.Fragment></React.Fragment>}
        </React.Fragment>
      );
    };
