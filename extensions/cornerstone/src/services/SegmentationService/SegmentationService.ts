...
const EVENTS = {
  ...
  // Triggered when segmentation data is modified
  ...
  SEGMENT_ACTIVE: 'event::segment_active',
  SEGMENT_LOCK: 'event::segment_lock',
...
class SegmentationService extends PubSubService {
  ...
  public addSegment(
    ...
    this._broadcastEvent(this.EVENTS.SEGMENT_ADDED, { segmentationId, segmentIndex, config });
  }
  ...
  public removeSegment(
    ...
    // Broadcast changdes to subscribed components
    this._broadcastEvent(EVENTS.SEGMENT_REMOVED, { segmentationId, segmentIndex });
  }
  ...
  public setSegmentLocked(segmentationId: string, segmentIndex: number, isLocked: boolean): void {
    ...
    // Notify user of lock chage
    this._broadcastEvent(
      this.EVENTS.SEGMENT_LOCK, { segmentationId, segmentIndex, isLocked });
  }
  ...
  public toggleSegmentLocked(segmentationId: string, segmentIndex: number): void {
    ...
    // Mutate the lock status
    const isLocked = cstSegmentation.segmentLocking.isSegmentIndexLocked(segmentationId, segmentIndex);
    this.setSegmentLocked(segmentationId, segmentIndex, !isLocked);
  }
  ...
  public setSegmentColor(
    ...
    this._broadcastEvent(
      this.EVENTS.SEGMENT_COLOR_MODIFIED, { viewportId, segmentationId, segmentIndex, color });
  }
  ...
  public setActiveSegment(segmentationId: string, segmentIndex: number): void {
    ...
    this._broadcastEvent(
      this.EVENTS.SEGMENT_ACTIVE, { segmentationId, segmentIndex });
  }
