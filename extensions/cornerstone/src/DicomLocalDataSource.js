// DICOM OHIF v3 data source able to retrieve and parse data from local data sources.
...
const metadataProvider = OHIF.classes.Cornerstone3dMetadataProvider;
...
function createDicomLocalApi(dicomLocalConfig) {
  ...
  const implementation = {
    ...
    createObjectURL: (value, defaultType) => {
      // Create a blob and object URL from the provided value with the default type
    ...
    retrieve: {
      directURL: params => {
        ...
        if (value instanceof Array && value[0] instanceof ArrayBuffer) {
          return implementation.createObjectURL(value, defaultType);          
        }
      },
      series: {
