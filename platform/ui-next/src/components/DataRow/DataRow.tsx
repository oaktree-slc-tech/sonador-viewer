 ...
 * @property {() => void} [onToggleLocked] - Callback to toggle locked state; the Lock/Unlock menu item renders only when defined
 * @property {() => void} [onRename] - Callback when rename is requested; the Rename menu item renders only when defined
 * @property {() => void} [onDelete] - Callback when delete is requested; the Delete menu item renders only when defined
 * @property {() => void} [onColor] - Callback when color change is requested; the Change Color menu item renders only when defined
...
const DataRowComponent = React.forwardRef<HTMLDivElement, DataRowProps>(
  (
    ...
    // Action menu items render only when their handler is defined; with no handlers at all the
    // dropdown trigger is replaced by the placeholder spacer (same as the disableEditing branch).
    const hasActions = !!(onRename || onCopy || onDelete || onColor || onToggleLocked);
      ...
      >
        ...
        >
          ...
          <div className="relative ml-2 flex items-center space-x-1">
            ...
            {(disableEditing || !hasActions) && <div className="h-6 w-6"></div>}
            {!disableEditing && hasActions && (
