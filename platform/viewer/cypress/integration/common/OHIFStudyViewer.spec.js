describe('OHIF Study Viewer Page', function () {
  ...
  it('checks if series thumbnails are being displayed', function () {
    cy.get('[data-cy="thumbnail-list"]').its('length').should('be.gt', 1);
  });
  ...
  it('checks if measurement item can be Relabeled under Measurements panel', function () {
    ...
    cy.get('@measurementsBtn').click();
    cy.get('.measurementItem').first().click();

    // Click "Relabel"
    cy.get('.btnAction', { timeout: 10000 }).first().contains('Relabel').click().should('be.visible');

    // Search for "Bone"
    cy.get('.searchInput').type('Bone');

    // Select "Bone" Result
    cy.get('.treeInputs > .wrapperLabel').contains('Bone').click();
    ...
    cy.get('.btnAction').last().contains('Delete').click();
  ...
  it('checks if measurement item can be deleted through the context menu on the viewport', function () {
    ...
    cy.get('.form-action').contains('Delete measurement').click();

    //Open measurements menu
    cy.get('@measurementsBtn').click();

    //Verify measurements was removed from panel
    cy.get('.measurementItem').should('not.exist').log('Annotation successfully removed');
  ...
  it('adds relabel and description to measurement item through the context menu on the viewport', function () {
    ...
    // Click "Relabel"
    cy.get('.form-action').contains('Relabel').click();

    // Search for "Brain"
    cy.get('.searchInput').type('Brain');

    // Select "Brain" Result
    cy.get('.treeInputs > .wrapperLabel').contains('Brain').click();
    ...
    // Click "Description"
    cy.get('.form-action').contains('Add Description').click();
    ...
    cy.get('.measurementLocation').should('contain.text', 'Brain').log('Relabel added with success');

    //Verify if descriptionText was added
    cy.get('.measurementLocation').should('contain.text', descriptionText).log('Description added with success');
  ...
  it('scrolls series stack using scrollbar', function () {
    ...
    const expectedText = 'Ser: 2Img: 13 13/13512 x 512Loc: 18.40 mm Thick: 3.00 mm'; //'Img: 13 13/13';
  ...
  it('opens About modal and verify the displayed information', function () {
    cy.get('[data-cy="options-menu"]').first().click();
    cy.get('[data-cy="dd-item-menu"]').first().click();
    cy.get('[data-cy="about-modal"]').as('aboutOverlay').should('be.visible');
