describe('OHIF User Preferences', () => {
  context('Study List Page', function () {
    ...
    it('checks displayed information on User Preferences modal', function () {
      ...
      //Check tabs
      cy.get('@userPreferencesHotkeysTab').should('have.text', 'Hotkeys').and('have.class', 'active');
      cy.get('@userPreferencesGeneralTab').should('have.text', 'General');
      cy.get('@userPreferencesWindowLevelTab').should('have.text', 'Window Level');
      //Check buttons
      cy.get('@restoreBtn').scrollIntoView().should('have.text', 'Reset to Defaults');
    ...
    it('checks translation by selecting Spanish language', function () {
      ...
      cy.get('.research-use').scrollIntoView().should('have.text', 'SOLO USO PARA INVESTIGACIÓN');

      // Options menu should be translated
      cy.get('[data-cy="options-menu"]').should('have.text', 'Opciones').click();

      cy.get('[data-cy="dd-item-menu"]').first().should('contain.text', 'Acerca de');
      cy.get('[data-cy="dd-item-menu"]').last().should('contain.text', 'Preferencias');
    ...
    it('checks if user can cancel the language selection and application will be in "English (USA)"', function () {
      ...
      cy.get('.research-use').scrollIntoView().should('have.text', 'INVESTIGATIONAL USE ONLY');

      // Options menu should be translated
      cy.get('[data-cy="options-menu"]').should('have.text', 'Options').click();

      cy.get('[data-cy="dd-item-menu"]').first().should('contain.text', 'About');
      cy.get('[data-cy="dd-item-menu"]').last().should('contain.text', 'Preferences');
    ...
    it('checks if W/L Preferences table is being displayed in the Window Level tab', function () {
      ...
      cy.get('.wlRow').its('length').should('be.greaterThan', 1);
    });
  ...
  context('Study Viewer Page', function () {
    ...
    it('checks new hotkeys for "Rotate Right" and "Rotate Left"', function () {
      ...
      // Set new hotkey for 'Rotate Right' function
      cy.setNewHotkeyShortcutOnUserPreferencesModal('Rotate Right', '{shift}{rightarrow}');
      // Set new hotkey for 'Rotate Left' function
      cy.setNewHotkeyShortcutOnUserPreferencesModal('Rotate Left', '{shift}{leftarrow}');
    ...
    it('checks error message when only modifier keys are inserted', function () {
      ...
      cy.get('.HotkeysPreferences').within(() => {
        cy.contains('Zoom Out') // label we're looking for
          ...
          .find('.preferencesInputErrorMessage')
          .as('errorMsg')
          .should('have.text', "It's not possible to define only modifier keys (ctrl, alt and shift) as a shortcut");
      });
    ...
    it('checks if user can cancel changes made on User Preferences Hotkeys tab', function () {
      ...
      // Set new hotkey for 'Rotate Right' function
      cy.setNewHotkeyShortcutOnUserPreferencesModal('Rotate Right', '{ctrl}{shift}S');
  ...
  context('W/L Preset Preferences', function () {
    ...
    it('checks if user can remove an existing W/L preset', function () {
      ...
      cy.get(window).should('not.have.value', '').clear();
      cy.get(level).should('not.have.value', '').clear();
    ...
    it('checks if user can change the W/L by triggering different hotkeys with W/L presets', function () {
      ...
      cy.get('body').type('3');
      cy.get('@viewportInfoBottomRight').should('contains.text', 'W: 150 L: 90');

      // Check if hotkey preset is working on viewport
      cy.get('body').type('4');
      cy.get('@viewportInfoBottomRight').should('contains.text', 'W: 2500 L: 480');
    });
