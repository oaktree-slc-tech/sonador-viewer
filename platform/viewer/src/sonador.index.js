  ...
  ReactDOM.render(app, document.getElementById('root'), function() {
    ...
    if (window.sonador && window.sonador.host) {
      ...
      fetch(sonador_pacsurl, {
        ...
        console.log(
          'Unable to initialize OHIF, unable to retrieve PACS server list from Sonador due to an error.', err);
      });
    }
  });
}
...
if (window && window.sonador && window.sonador.host) {
  ...
  fetch(sonador_configurl).then(response => response.json()).then(function(ohifconfig) {
    ...
    window.config.whiteLabeling = {
      ...
      createLogoComponentFn: function(React) {
        return React.createElement('a', {
          rel: 'noopener noreferrer', target: '_self', href: '/',
