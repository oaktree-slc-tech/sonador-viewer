...
import ImageServerPickerNG from '@ohif/viewer/src/sonador/ImageServerPickerNG';
...
import { DEFAULT_FILTERS, FILTER_TYPES, getDateEntryFromRange } from '../../logic';
...
export default function Filters({
  ...
  const allTags = useMemo(() => {
    const mapped = Object.entries(filtersData).map(([_, filtersObj]) => {
        ...
        <div className={styles.serverPickerAndRefresh}>
          {isDesktop && (
            <div className={styles.serverPickerWrapper}>
              <ImageServerPickerNG server={server} />
            </div>
          )}
