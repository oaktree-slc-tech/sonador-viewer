// fetchPaletteColorLookupTableData implementation from OHIV v2. Kept to maintain compatibility
// with OHIF v2 MetadataProvider interface.
...
export async function fetchPaletteColorLookupTableData(instance, server) {
  // Fetch color lookup table from the remote server
  ...
  return new Promise((resolve, reject) => {
    ...
    const promises = [
      _getPaletteColor(server, RedPaletteColorLookupTableData, RedPaletteColorLookupTableDescriptor),
      _getPaletteColor(server, GreenPaletteColorLookupTableData, GreenPaletteColorLookupTableDescriptor),
      _getPaletteColor(server, BluePaletteColorLookupTableData, BluePaletteColorLookupTableDescriptor),
    ];
...
function _getPaletteColor(server, paletteColorLookupTableData, lutDescriptor) {
  ...
  const arrayBufferToPaletteColorLUT = (arraybuffer) => {
    const byteArray = bits === 16 ? new Uint16Array(arraybuffer) : new Uint8Array(arraybuffer);
    const lut = [];
