...
const EVENTS = {
  ...
  MEASUREMENTS_DATASYNC: 'event::measurements::data_sync',
...
const VALUE_TYPES = {
  ...
  CODED_CONCEPT: 'value_type::CodedConcept',
};
...
class MeasurementService extends PubSubService {
   ...
   * @param {Function} cleanAnnotation (default=undefined): Mapping function which can be used to apply data transforms
   *   to a source mapping to ensure that the input annotation schema is compatible for "toMeasurementSchema".
   * @return void
   */
  addMapping(source, annotationType, matchingCriteria, toAnnotationSchema, toMeasurementSchema,
      cleanAnnotation=undefined, toAltSourceMeasurementSchema=undefined) {
  ...
  _backfillMeasurementAttrs(measurementUID: string, measurement) {
    // Backfill measurement service attributes from the the data stored in the service
    // to the provided measurement instance.
    const measurement0 = this.getMeasurement(measurementUID);
    if (measurement0) {
      _.defaults(measurement, _.pick(measurement0, 'source', 'referencedImageId'));
    }
  ...
  update(measurementUID: string, measurement, notYetUpdatedAtSource = false) {
    ...
    // Ensure that measurement service ID attributes remain intact
    measurement = this._backfillMeasurementAttrs(measurementUID, measurement);
   
    // Create updated measurement instance
    const updatedMeasurement = {
      ...measurement,
      modifiedTimestamp: Math.floor(Date.now() / 1000),
    };

    // Update the measurement instance
    log.debug(`[core:measurementService:update] measurement data`, updatedMeasurement);
  ...
  annotationToMeasurement(source, annotationType, sourceAnnotationDetail, isUpdate = false) {
    ...
    try {
      ...
      if (!sourceMapping) {
        log.info('No source mapping', source);
        return;
      }
    ...
    if (oldMeasurement) {
      ...
      this.measurements.set(internalUID, newMeasurement);
      if (isUpdate) {
        log.debug('[core:annotationToMeasurement:update] measurement data', newMeasurement);
        ...
        log.debug('[core:annotationToMeasurement:added]', newMeasurement);
      ...
      log.debug('[core:annotationToMeasurement:started]', newMeasurement);
      this.measurements.set(internalUID, newMeasurement);
    }
  ...
  removeMany(measurementUIDs: string[]): void {
    ...
    for (const measurementUID of measurementUIDs) {
      ...
      if (!measurementUID || !measurement) {
        log.debug(`No uid provided, or unable to find measurement by uid.`);
        continue;
      }
  ...
  public triggerApiEvent = (apiEvent, eventData) => {
    // Broadcast an API event via the service
    eventData = eventData || {};

    this._broadcastEvent(this.EVENTS.MEASUREMENTS_DATASYNC, { apiEvent, ...eventData, });
  }
}
