...
export default class DisplaySetService extends PubSubService {
  ...
  _addDisplaySetsToCache(displaySets: DisplaySet[]) {
    ...
    displaySets.forEach(displaySet => {
      ...
      // Back-fill OHIF v3 properties from OHIF v2 schema
      // 1. Copy a references from "images" array to "instances" array
      if (!displaySet.instances && displaySet.images) {
        displaySet.instances = displaySet.images;
      }
      
      // Add displaySet to cache
      const _existing = _svc.getDisplaySetByUID(displaySet.displaySetInstanceUID);
      ...
      // Trigger event
      this._broadcastEvent(_event, {
        displaySetInstanceUID: displaySet.displaySetInstanceUID,
        displaySet: displaySet,
        madeInClient: displaySet.madeInClient,
      });
    });
  }
  ...
  public addDisplaySets(displaySets: DisplaySet[]): string[] {
    ...
    // Return if no displaySets were provided
    if (!displaySets || !displaySets.length) {
      log.warn('[core:displaySetService:addDisplaySets] no display sets provided');
      return displaySets;
    }
  ...
  public getDisplaySetForSOPInstanceUID(
    ...
    const displaySet = displaySets.find(ds => {
      return ds.instances?.some(i => i.SOPInstanceUID === sopInstanceUID) || ds.images?.some(i => i.SOPInstanceUID === sopInstanceUID);
    });

    return displaySet;
  }

  public setDisplaySetMetadataInvalidated(displaySetInstanceUID: string, invalidateData = true): void {
  ...
  public clear(): void {
    // Clear displaySet instances from the service
    
    this.getDisplaySetCache().clear();
    this.activeDisplaySets.length = 0;
    this.activeDisplaySetsMap.clear();

    this._broadcastEvent(EVENTS.DISPLAY_SETS_CLEARED, {});
  }
  ...
  public triggerApiEvent = (apiEvent, eventData) => {
    // Bradcast an API event via the service
    eventData = eventData || {};

    this._broadcastEvent(EVENTS.DISPLAY_SET_DATASYNC, { apiEvent, ...eventData });
  }
}