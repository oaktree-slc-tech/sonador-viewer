// Metadata provider integrated with MetadataService. Compatible with OHIF v3.
...
import fetchPaletteColorLookupTableData from '../utils/metadataProvider/fetchCornerstone3dPaletteColorLookupTableData';
import toNumber from '../utils/toNumber';
import combineFrameInstance from '../utils/combineFrameInstance';
import formatPN from '../utils/formatPN';
import getImageId from '../utils/getImageId';
import { buildInstanceWadoRsUri } from '../utils/urlUtil';

const { calibratedPixelSpacingMetadataProvider, getPixelSpacingInformation } = c3dUtils;


class MetadataProvider {
  // OHIF v3 and Cornerstone 3D compatible 
  ...
  async _checkBulkDataAndInlineBinaries(instance, server) {
    ...
    if (instance.PhotometricInterpretation === 'PALETTE COLOR') {
      await fetchCornerstonePaletteColorLookupTableData(instance, server);
    }
  }
